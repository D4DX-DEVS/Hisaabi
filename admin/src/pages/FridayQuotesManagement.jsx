...
import { Plus, Pencil, Trash2, X, Save, AlignLeft, Eye, Search, Quote, Link } from 'lucide-react';
...
const EMPTY = { text: '', malayalam: '', english: '', urdu: '', reference: '' };
      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all w-full max-w-2xl relative z-10 animate-slide-in-right flex flex-col max-h-[90vh]">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 font-display">{item ? 'Edit Friday Quote' : 'Add Friday Quote'}</h2>
              <p className="text-xs text-slate-500 font-medium">{item ? 'Update quote content' : 'Add a new Friday quote'}</p>
            </div>
          </div>
...
export default function FridayQuotesManagement() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['admin-friday-quotes', page, debouncedSearch],
    queryFn: () => adminApi.getFridayQuotes({
      page,
      limit: LIMIT,
  ...
  const createMutation = useMutation({
    mutationFn: (d) => adminApi.createFridayQuote(d),
    onSuccess: () => {
      qc.invalidateQueries(['admin-friday-quotes']);
      setSuccessState({ isOpen: true, title: 'Success!', message: 'Friday quote created successfully.' });
      setModal(null);
    },
    onError: (e) => toast.error(e.response?.data?.error || 'Failed to create Friday quote'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => adminApi.updateFridayQuote(id, data),
    onSuccess: () => {
      qc.invalidateQueries(['admin-friday-quotes']);
      setSuccessState({ isOpen: true, title: 'Success!', message: 'Friday quote updated successfully.' });
      setModal(null);
    },
    onError: (e) => toast.error(e.response?.data?.error || 'Failed to update Friday quote'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => adminApi.deleteFridayQuote(id),
    onSuccess: () => {
      qc.invalidateQueries(['admin-friday-quotes']);
      setSuccessState({ isOpen: true, title: 'Deleted!', message: 'Friday quote removed successfully.' });
      setItemToDelete(null);
    },
    onError: (e) => {
      toast.error(e.response?.data?.error || 'Failed to delete Friday quote');
      setItemToDelete(null);
    },
  });
  ...
  const quotes = data?.friday_quotes || [];
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col min-h-0 gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-2 ml-1">
              {total > 0 ? `${total} quotes in the catalogue` : 'Manage Friday quotes'}
            </p>
          </div>
          <button
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-24">
            ...
            <p className="font-bold text-slate-500">Loading Friday quotes...</p>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-24 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-700 font-display mb-2">No Friday Quotes Yet</h3>
            <p className="text-slate-500 max-w-sm mx-auto">Add the first Friday quote to the catalogue.</p>
            <button
              onClick={() => setModal('create')}
                    ...
                    {quotes.map((quote) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 max-w-xs">
                          <p className="text-slate-500 text-xs font-medium truncate">{quote.reference || '—'}</p>
                        </td>
                        <td className="px-6 py-4">
      ...
      <ConfirmationModal
        ...
        title="Delete Friday Quote"
        message={`Are you sure you want to delete this Friday quote? This action cannot be undone.`}
