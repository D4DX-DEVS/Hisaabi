...
import { Plus, Pencil, Trash2, X, Moon, Save, AlignLeft, Search, ChevronDown, ChevronRight, FolderOpen, Folder, FileText, Tag, Filter, Hash } from 'lucide-react';
import { adminApi } from '../api/adminApi';
import ConfirmationModal from '../components/ConfirmationModal';
import SuccessModal from '../components/SuccessModal';

const EMPTY = { name: '', display_name: '', description: '', parent: '', order: '' };
...
function Modal({ item, mainTypes, defaultParent, onClose, onSave }) {
  ...
  const [form, setForm] = useState(
    item
      ? { name: item.name || '', display_name: item.display_name || '', description: item.description || '', parent: initialParent, order: item.order ?? '' }
                        ...
                        <>
                          <tr key={typeId} className="hover:bg-slate-50/80 transition-colors group bg-white">
                            <td className="px-4 py-4 text-center">
                              {type.order != null ? (
                                <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-indigo-50 text-indigo-700 text-xs font-bold border border-indigo-200">{type.order}</span>
                              ) : (
