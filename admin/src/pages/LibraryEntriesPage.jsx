...
import { Plus, Pencil, Trash2, X, LibraryBig, Save, AlignLeft, Eye, Search, ChevronLeft, Hash } from 'lucide-react';
...
const EMPTY_ENTRY = { title: { ...EMPTY_TRILINGUAL }, description: { ...EMPTY_TRILINGUAL }, content: '', source: '', order: '' };
...
function normTitle(raw) {
  if (!raw || typeof raw !== 'object') return { malayalam: typeof raw === 'string' ? raw : '', english: '', urdu: '' };
  return { malayalam: raw.malayalam ?? '', english: raw.english ?? '', urdu: raw.urdu ?? '' };
}

// Strips HTML tags to test/display plain-text content (e.g. emptiness checks, table previews).
...
function EntryModal({ item, categoryName, onClose, onSave }) {
  ...
  const setContent = (e) => setForm((p) => ({ ...p, content: e.target.value }));
              ...
              {LANG_FIELDS.map(({ key, label, required }) => (
                <div key={key}>
                  ...
                  <RichTextEditor
                    ...
                    onChange={(html) => setForm((p) => ({ ...p, description: { ...p.description, [key]: html } }))}
...
function ViewModal({ item, categoryName, onClose }) {
  if (!item) return null;
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-0">
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity animate-fade-in" onClick={onClose} />
              ...
              {[{ key: 'malayalam', label: 'Malayalam' }, { key: 'english', label: 'English' }, { key: 'urdu', label: 'Urdu' }].map(({ key, label }) =>
                item.title?.[key]?.trim() ? (
                  <div key={key}>
                    ...
                    <div dir={key === 'urdu' ? 'rtl' : undefined} className={`text-slate-800 font-semibold bg-slate-50 px-4 py-3 rounded-xl border border-slate-100 text-sm${key === 'urdu' ? ' text-right font-arabic' : ''}`}>
                      {item.title[key]}
                    </div>
                  </div>
                ) : null
              ...
              {[{ key: 'malayalam', label: 'Malayalam' }, { key: 'english', label: 'English' }, { key: 'urdu', label: 'Urdu' }].map(({ key, label }) =>
                item.description[key] ? (
                  <div key={key}>
                    ...
                    <div
                      dir={key === 'urdu' ? 'rtl' : undefined}
                      className={`rich-text-content text-slate-700 bg-slate-50 px-4 py-3 rounded-xl border border-slate-100 text-sm leading-relaxed min-h-[80px] overflow-auto${key === 'urdu' ? ' text-right font-arabic' : ''}`}
                      dangerouslySetInnerHTML={{ __html: item.description[key] }}
                    />
                  </div>
                ) : null
              )}
...
export default function LibraryEntriesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-library-entries', categoryId],
    queryFn:  () => adminApi.getLibraryEntries(categoryId, { limit: 500 }).then((r) => r.data),
    enabled:  !!categoryId,
  });

  const createMutation = useMutation({
    mutationFn: (d) => adminApi.createLibraryEntry(categoryId, d),
    onSuccess: () => {
      qc.invalidateQueries(['admin-library-entries', categoryId]);
      setSuccessState({ isOpen: true, title: 'Created!', message: 'Entry created successfully.' });
      setModal(null);
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => adminApi.updateLibraryEntry(id, data),
    onSuccess: () => {
      qc.invalidateQueries(['admin-library-entries', categoryId]);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => adminApi.deleteLibraryEntry(id),
    onSuccess: () => {
      qc.invalidateQueries(['admin-library-entries', categoryId]);
  ...
  const handleSave = (form) => {
    if (!stripHtml(form.description?.malayalam)) { toast.error('Malayalam description is required'); return; }
    if (modal === 'create') createMutation.mutate(form);
    else updateMutation.mutate({ id: modal.id, data: form });
  };

  const categoryName = data?.category?.name || 'Library';
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col min-h-0 gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <button
              ...
              className="flex items-center gap-1.5 text-slate-500 hover:text-indigo-600 text-sm font-medium mb-2 transition-colors"
            >
            ...
            <p className="text-slate-500 text-sm mt-2 ml-1">Manage the {allItems.length} entries in this category.</p>
          </div>
          <button
                    ...
                    {items.map((item) => (
                      <tr key={item.id} onClick={() => setViewItem(item)} className="hover:bg-slate-50/80 transition-colors group cursor-pointer">
                        <td className="px-4 py-4 text-center">
                          {item.order != null ? (
                            <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-indigo-50 text-indigo-700 text-xs font-bold border border-indigo-200">{item.order}</span>
                          ) : (
                        ...
                        <td className="px-6 py-4 max-w-[200px]">
                          <p className="text-slate-800 font-medium text-sm truncate">
                            {item.title?.malayalam?.trim() || item.title?.english?.trim() || '—'}
                          </p>
                        ...
                        <td className="px-6 py-4 max-w-xs">
                          <p className="text-slate-700 text-sm leading-relaxed truncate">
                            {(() => {
                              const plain = stripHtml(item.description?.malayalam);
      ...
      <ViewModal item={viewItem} categoryName={categoryName} onClose={() => setViewItem(null)} />
    </div>
  );
}
