...
import { Plus, Pencil, Trash2, X, Tag, Save, AlignLeft, Search, ChevronDown, ChevronRight, FolderOpen, Folder, FileText, Hash } from 'lucide-react';
import { adminApi } from '../api/adminApi';
import ConfirmationModal from '../components/ConfirmationModal';
import SuccessModal from '../components/SuccessModal';

const EMPTY = { name: '', display_name: '', display_name_malayalam: '', display_name_urdu: '', description: '', parent: '', order: '' };
  ...
  const [form, setForm] = useState(
    item
      ? {
          ...
          display_name_malayalam: item.display_name_malayalam || '',
          display_name_urdu: item.display_name_urdu || '',
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                {isEdit ? 'Update dhikr category details' : (form.parent ? 'Add a sub-category under a main category' : 'Add a new main dhikr category')}
              </p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              value={form.name}
              onChange={set('name')}
              placeholder={form.parent ? 'e.g. morning_adhkar' : 'e.g. daily_adhkar'}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder={form.parent ? 'e.g. Morning Adhkar' : 'e.g. Daily Adhkar'}
            />
          </div>

          {/* Display Name translations (optional) */}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="flex items-center gap-1.5 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                <Tag size={14} /> Display Name (Malayalam)
              </label>
              <input
                type="text"
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-medium bg-slate-50 focus:bg-white transition-colors"
                value={form.display_name_malayalam}
                onChange={set('display_name_malayalam')}
                placeholder="Optional"
              />
            </div>
            <div>
              <label className="flex items-center gap-1.5 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                <Tag size={14} /> Display Name (Urdu)
              </label>
              <input
                type="text"
                dir="rtl"
                ...
                value={form.display_name_urdu}
                onChange={set('display_name_urdu')}
                placeholder="اختياري"
              />
            </div>
          </div>
...
export default function DhikrCategoriesManagement() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => adminApi.getDhikrCategories({ limit: 500 }).then((r) => r.data),
  });

  const createMutation = useMutation({
    mutationFn: (d) => adminApi.createDhikrCategory({ ...d, parent: d.parent || null }),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dhikr-categories']);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => adminApi.updateDhikrCategory(id, { ...data, parent: data.parent || null }),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dhikr-categories']);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => adminApi.deleteDhikrCategory(id),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dhikr-categories']);
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide">
        {isLoading ? (
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-24 text-center">
            ...
            <p className="text-slate-500 max-w-sm mx-auto">Add the first dhikr category to get started.</p>
            <button
              onClick={() => { setDefaultParent(''); setModal('create'); }}
                          ...
                          <tr key={catId} className="hover:bg-slate-50/80 transition-colors group bg-white">
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold text-slate-800 text-base">{cat.display_name}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <span className="font-mono font-bold text-slate-600 text-sm bg-slate-100 px-2 py-0.5 rounded-lg">{cat.name}</span>
                            </td>
                            <td className="px-6 py-4">
                              {subs.length > 0 ? (
                                <button
